...
export default function DrawTable({ draws }: DrawTableProps) {
  ...
  const digitCount = Math.max(3, ...draws.map((draw) => getDigitsFromDraw(draw).length));
  const digitHeaders = Array.from({ length: digitCount }, (_, i) => `D${i + 1}`);
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-gray-800 text-left">
            {['Date', 'Label', 'Result', ...digitHeaders, 'Local Time', 'UTC', 'Confidence', ''].map((h) => (
          ...
          {draws.map((draw) => {
            ...
            return (
              <tr key={draw.draw_id as string} className="hover:bg-gray-800/40 transition-colors">
                ...
                {digitHeaders.map((_, index) => (
                  <td key={index} className="px-3 py-2.5 font-mono text-gray-300">
                    {digits[index] ?? '—'}
                  </td>
                ))}
